...
const releaseSeatsAndDeleteBookings = inngest.createFunction(
    ...
    async ({event, step}) => {
        ...
        await step.run('check-payment-status', async() => {
            ...
            //if payment is not made  relese seats and delete bookings
            if(!releaseSeatsAndDeleteBookings.isPaid) {
                const show = await Show.findById(booking.show);
                booking.bookedSeats.forEach((seat)=>{
                    delete show.occupiedSeats[seat]
                });
                show.markModified('Occupies Seats')
                await show.save();
        ...
        await sendEmail({
            ...
            body: ` <div style="font-family: Arial,     sans-serif; line-height: 1.5;">
             <h2>Hello ${booking.user.name},</h2>
              <p>Your booking for <strong style="color: #F84565;"> "${booking.show.movie.title}"</strong> is confimed.</p>
               <p>
    ...
    async ( step) => {
        ...
        const reminderTasks = await step.run("prepare-reminder-tasks", async () => {
            ...
            for(const show of shows){
                if(!show.movie || !occupiedSeats) continue;

                const userIds = [...new Showet(Object.values(show.occupiedSeats))];
        ...
        const results = await step.run('sent-all-reminders', async ()=> {
            return await Promise.allSettled(
                reminderTasks.map(task => sendEmail({
                    to: task.userEmail,
                    sub: `Reminder: Your movie "${task.movieTitle} starts soon!`,
    ...
    async ({event})=> {
        ...
        for(const user of users) {
            ...
            const body = ` <div style="font-family: Arial,     sans-serif; padding:20px;">
                        <h2>Hi ${userName},</h2>
                        <p>We've just added a new show to out library:</p>
